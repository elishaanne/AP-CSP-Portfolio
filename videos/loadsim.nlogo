...
to eat-rabbits
  ...
  if prey != nobody
  [ ask prey [ die ]
    set energy energy + foxes-energy  ]
end

to reproduce-rabbits
  if random-float 100 < rabbits-reproduce [
    set energy (energy / 2)
...
to reproduce-foxes
  if random-float 100 < foxes-reproduce [
    set energy (energy / 2)
    hatch 1 [ rt random-float 360 fd 1 ]
  ]
end
...
"foxes" 1.0 0 -8431303 true "" "plot count foxes"
